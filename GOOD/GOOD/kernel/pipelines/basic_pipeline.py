...
class Pipeline:
    ...
    def train_batch(self, data: Batch, pbar) -> dict:
        ...
        loss = self.ood_algorithm.loss_calculate(raw_pred, targets.float(), mask, node_norm, self.config)
    ...
    def train(self):
        ...
        # train the model
        print(self.config.train.ctn_epoch, self.config.train.max_epoch)
        for epoch in range(self.config.train.ctn_epoch, self.config.train.max_epoch):
            ...
            # checkpoints save
            self.save_epoch(epoch, val_stat, self.config)
    ...
    def evaluate(self, split: str):
        ...
        for data in pbar:
            with torch.no_grad():
                ...
                # --------------- Loss collection ------------------
                loss: torch.tensor = self.config.metric.loss_func(raw_preds, targets.float(), reduction='none') * mask
    ...
    def config_model(self, mode: str, load_param=False):
        ...
        if mode == 'test':
            ...
            if os.path.exists(self.config.id_test_ckpt):
                ...
                print(
                    f'#IN#ChartInfo {ckpt["val_score"]:.4f}', end='')
            if load_param:
                if self.config.ood.ood_alg != 'EERM':
                    self.model.load_state_dict(ckpt['state_dict'])
                else:
                    self.model.gnn.load_state_dict(ckpt['state_dict'])
            return ckpt["val_score"], ckpt["val_loss"]

    def save_epoch(self, epoch: int, val_stat: dir,
                   config: Union[CommonArgs, Munch]):
        r"""
