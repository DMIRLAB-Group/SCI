...
from .GINs import GINMolEncoder, GINFeatExtractor, GINEConv
from .GINvirtualnode import VirtualNodeEncoder, vGINEncoder
...
class SCI2(GNNBasic):
    def __init__(self, config: Union[CommonArgs, Munch]):
        ...
        self.z5 = GINEConv(nn.Sequential(nn.Linear(self.dim_hidden, 2 * self.dim_hidden),
                                            nn.BatchNorm1d(2 * self.dim_hidden), nn.ReLU(),
                                           nn.Dropout(p=self.dropout),
                                            nn.Linear(2 * self.dim_hidden, self.dim_hidden)))
        ...
        self.mu = nn.Linear(self.emb_dim, self.S_emb_dim)
        self.logvar = nn.Linear(self.emb_dim, self.S_emb_dim)
        self.A_new_MLP = nn.Sequential(nn.Linear(2, 1),
                                       nn.Sigmoid())
        self.classifier = Classifier(config)

    def mlp_encode(self, x_emb, edge_index, edge_attr):
        emb = self.z5(x_emb, edge_index, edge_attr)
        return self.mu(emb), self.logvar(emb)
