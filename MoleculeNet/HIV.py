...
parser.add_argument('--path', type=str, default='data/HIV.csv', help='which path ')
...
def train(model, dataset, optimizer, loss_function):
    ...
    for counter, train_batch in enumerate(batch_list):
        ...
        x_atom, x_bonds, x_atom_index, x_bond_index, x_mask, smiles_to_rdkit_list, adj , node_y = get_smiles_array(smiles_list,
                                                                                                     feature_dicts)
        atoms_adj, g_A, gh, gn, z2 ,z2_A , x1, k1, mu, logvar, gh_kl, gn_kl, mol_prediction = model(torch.Tensor(x_atom), torch.Tensor(x_bonds),
        ...
        losstotal = Loss_function(adj, atoms_adj, g_A, gh, gn, z2, z2_A, x1, k1, x_atom, node_y, mu, logvar, gh_sparity,
                                  gn_sparity, gh_kl, gn_kl, args.KL_para, args.gn_sparity_loss_para, args.gh_sparity_loss_para,
                                  args.g1_loss_para, args.x1_loss_para, args.k1_loss_para, args.Lr_para, args.batch_size, args.device)
...
def eval(model, dataset):
    ...
    for counter, eval_batch in enumerate(batch_list):
        ...
        x_atom, x_bonds, x_atom_index, x_bond_index, x_mask, smiles_to_rdkit_list ,adj ,node_y= get_smiles_array(smiles_list,
                                                                                                     feature_dicts)
        atoms_prediction, g_A, gh, gn, z2, z2_A, x1, k1, _, _, gh_kl, gn_kl, mol_prediction = model(torch.Tensor(x_atom), torch.Tensor(x_bonds),
                                                 torch.cuda.LongTensor(x_atom_index),
                                                 torch.cuda.LongTensor(x_bond_index), torch.Tensor(x_mask), torch.Tensor(adj), torch.Tensor(node_y))
        for i, task in enumerate(tasks):
            y_pred = mol_prediction[:, i * per_task_output_units_num:(i + 1) *
...
# for classification model with 2 classes
