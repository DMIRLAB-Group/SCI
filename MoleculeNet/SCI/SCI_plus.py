...
class SCI_model_plus(nn.Module):
    ...
    def __init__(self, radius, T, input_feature_dim, input_bond_dim, \
        ...
        super(SCI_model_plus, self).__init__()
        ...
        self.radius = radius
        self.T = T
        self.z1_gcn = nn.ModuleList([DenseGCNConv(in_channels=fingerprint_dim, out_channels=self.emb_dim1),
                                     DenseGCNConv(in_channels=self.emb_dim1, out_channels=self.emb_dim1)])
        self.z2_dc = BatchInnerProductDecoder()
        self.z3_gcn = nn.ModuleList([DenseGCNConv(in_channels=fingerprint_dim, out_channels=self.emb_dim1),
                                     DenseGCNConv(in_channels=self.emb_dim1, out_channels=self.emb_dim1)])
        self.z4_dc = BatchInnerProductDecoder()
        self.X1_gcn = nn.ModuleList([DenseGCNConv(in_channels=fingerprint_dim, out_channels=self.emb_dim1),
                                     DenseGCNConv(in_channels=self.emb_dim1, out_channels=self.emb_dim1)])
    ...
    def mlp_encode(self, fingerprint_dim):
        ...
        return self.mu_linear(fingerprint_dim), self.logvar_linear(fingerprint_dim)
