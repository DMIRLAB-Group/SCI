...
class SCI_model_plus(nn.Module):
    ...
    def forward(self, x, y, adjj, node_yy, atom_degree_list, bond_degree_list, atom_mask):
        atom_mask = atom_mask.unsqueeze(2)
        mask_adj = torch.where(node_yy == -1, 0, 1).squeeze(-1)
        batch_size, mol_length, num_atom_feat = x.size()
        atom_feature = F.leaky_relu(self.atom_fc(x))
        adj = adjj
        # encoder
        ...
        bond_neighbor = [y[i][bond_degree_list[i]] for i in range(batch_size)]
        bond_neighbor = torch.stack(bond_neighbor, dim=0)
        atom_neighbor = [x[i][atom_degree_list[i]] for i in range(batch_size)]
